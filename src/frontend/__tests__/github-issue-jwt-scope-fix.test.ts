...
describe('GitHub Issue JWT Scope Fix Validation', () => {
  ...
  it('should fix the issue: JWT tokens now include all 5 required scopes', () => {
    // Setup: Use client config with all required scopes (as returned by /api/clientConfig)
    const clientConfigWithCorrectScopes = {
      azure: {
    ...
    setCachedClientConfig(clientConfigWithCorrectScopes);
  ...
  it('should clearly indicate when client config is not available', () => {
    // Test edge case where client config is not loaded
    clearCachedClientConfig();
