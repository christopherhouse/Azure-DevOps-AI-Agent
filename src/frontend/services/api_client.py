    ...
    async def send_chat_message(
        ...
        try:
            ...
            async with AsyncClient(timeout=self.timeout) as client:
                ...
                logger.info(f"Chat message sent successfully, got response: {len(result.get('data', {}).get('response', ''))} chars")
                return result
                
        except HTTPStatusError as e:
            logger.error(f"HTTP error sending chat message: {e.response.status_code} - {e.response.text}")
